...
def point_panel(ax, labels, means, stds, colors, ylim, title, delta_text=None, ylabel=False):
...
def main() -> None:
    ...
    for off, lab, color, y0, e0, y1 in zip(offsets, labels, colors, centered, centered_err, strict):
    ...
    ax.set_ylabel("MAE (min, lower is better)")
    ax.grid(axis="y", color="#e5e7eb", linewidth=0.8)
    ax.legend(loc="upper left", frameon=False, ncol=3, bbox_to_anchor=(0.08, 1.02), handletextpad=0.3, columnspacing=0.8)
    ax.text(0.98, 0.08, "strict: 0.08 min range", transform=ax.transAxes, ha="right", va="bottom",
