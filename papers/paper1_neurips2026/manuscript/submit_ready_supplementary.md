---
title: "Supplementary Material: When Does Workflow Conditioning Help Remaining Surgery Duration Prediction? A Variability-Scaling Study on MultiBypass140 and Cholec80"
---
...
20. Pedregosa, F., et al. (2011). scikit-learn: Machine learning in Python.
    *Journal of Machine Learning Research*, 12, 2825–2830. (Source of the
    `IsotonicRegression` used in Appendix Z.)
...
(at $2.29/hr). The reproducibility package (Appendix Z) re-runs any single
...
The headline result for this control appears inline in Appendix Z: shuffled
...
This appendix contains material that was condensed or omitted from the main
...
not benchmark-clean**, and we treat them as such throughout Appendix Z.
...
appears in Appendix Z and Appendix C.
...
Adam-style smoothing (§4.6), and bootstrap/Wilcoxon statistics (Appendix Z). All
...
| **Run 037** shuffled-token | Parameter-capacity confound (was the gain just from supplying any extra learnable token?) | Cluster IDs randomly permuted across videos (`random.Random(0).shuffle`); marginal preserved | **13.14 min ≈ no-token 13.03; ≠ oracle 12.18** | Appendix Z — semantic specificity confirmed |
...
> **What is in this paper.** §6.1–Appendix Z report fold-0 strict-protocol
> training numbers (Run 033 + Run 034 + Run 037). §6.5 reports the
> deployable pixel-only causal evaluation (Run 035 + Run 044 + Run 045).
> Appendix Z reports the secondary Cholec80 absolute number. Appendix Z contains
...
variability-scaling reading (Appendix Z): when the benchmark contains real
...
with the shuffled-token experiment in Appendix Z below.
...
than no-token under matched metric) confirms the Appendix Z semantic control:
...
post-processing in Appendix Z), a one-page [`QUICKSTART.md`](reproducibility/QUICKSTART.md)
...
- [`scripts/verify_cholec80_3.56.sh`](reproducibility/scripts/verify_cholec80_3.56.sh)
  — one-command reproduction of the Appendix Z Cholec80 ensemble result.
...
orthogonal to workflow conditioning, Appendix Z);
...
| Inference-time stack (test) | **3.56** | (3-seed ensemble + TTA + isotonic, secondary Appendix Z) | | |
