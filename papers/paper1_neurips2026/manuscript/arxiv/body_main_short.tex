...
MultiBypass140, cross-center MultiBypass140, and Cholec80. The primary
within-center MB140 result uses fold 0 with 3 seeds; because fold 0 is
...
\midrule
MB140 within-center primary & Fold-0 train/val split within MB140 & Strict prefix-only & no-token; retrospective oracle; decoupled-oracle & 3 & Run 033 \\
...
The paper makes four contributions. First, we show a consistent pattern
across three settings: workflow information helps on the more
workflow-diverse MultiBypass140 benchmark, remains useful under
cross-center transfer, and provides little or no benefit on the more
standardized Cholec80 benchmark. Here, workflow information refers to
the overall course of the surgery, especially the sequence of major
operative phases. Second, we compare a real-time evaluation setting that
uses only past frames with a legacy setting that also includes future
frames, and show that the real-time setting reveals stronger
workflow-conditioning gains. Third, we run a control experiment in which
workflow labels are randomly reassigned to the wrong surgeries. Under
this mismatch, the gain disappears, showing that the improvement comes
from correct workflow-conditioning information rather than from merely
adding a learnable workflow token to the model input. Fourth, we test a
more realistic deployment setting in which the model must infer the
workflow signal from the video seen so far, rather than receiving it
from the full surgery. In this setting, part of the
workflow-conditioning gain remains.
...
\(8 \times 768\) tensor per 8-frame clip, as shown in Figure~\ref{fig:paper-overview}. \textbf{Workflow token.} Each surgery
...
We report the primary within-center result on MultiBypass140 fold 0
...
training and inference, so these numbers are diagnostic evaluation results
...
decoupled-oracle row is the lowest nominal fold-0 MAE, though its 0.08
min margin over the standard oracle is within combined seed-level noise
($\approx$0.17 min). It differs from the standard oracle by decoupling
the phase head from the workflow-cluster token, closing the
phase/cluster circularity discussed in Limitations; we adopt this
...
pre-temporal-mixing visual features, so the workflow gain cannot be
attributed to the auxiliary head reading the cluster token. This decoupled spatial-then-temporal
...
To test whether the fold-0 result generalizes, we ran the
...
gain shrinks to \textbf{$-$0.19 min}, with negative $\Delta$ on folds 0, 1,
and 4 and positive $\Delta$ on folds 2 and 3. Per-fold MAE varies
from 8.68 to 13.03 min, indicating substantial case-mix heterogeneity.
We retain fold 0 as the development-fold result and report the 5-fold
table for transparency; per-seed numbers are in Appendix C.1.
...
a clear negative result. On a benchmark where 71\% of videos belong to a
...
the released \texttt{evaluate\_causal\_rsd\_pixel\_only} evaluator
...
the strict prefix-only protocol confirm both primary gains. On
...
\subsection{Implications of the variability-scaling result}\label{significance-what-the-variability-scaling-result-actually-buys-the-field}
...
constitute diagnostic evaluation results under a deployment-relevant clip
...
code (\S\ref{inference-causal-at-inference-variant}). The shuffled-token semantic control (13.14
...
\textbf{Benchmark choice matters for workflow conditioning.} Cholec80 has
carried phase-recognition and remaining-duration benchmarking for nearly
a decade. Our results show that, for the
\emph{workflow-conditioning question} specifically, Cholec80 has limited
residual workflow signal: 71\% of its videos belong to a single workflow
cluster. A method designed to exploit workflow variation may therefore
appear ineffective on Cholec80 even when the same mechanism is useful on
multicenter benchmarks. MultiBypass140, and similar multi-center
benchmarks as they become available, are better current testbeds for
this question, with Cholec80 retained as a useful low-variability
contrast case.
...
Workflow conditioning helps RSD when the dataset has meaningful workflow
variation and adds little under a single-template benchmark. Under the
strict prefix-only protocol, conditioning improves MB140 within-center
and cross-center RSD while Cholec80 remains null; the causal-at-inference
evaluator shows part of the gain survives prefix-only inference.
Closing training-time alignment is the natural follow-up.
