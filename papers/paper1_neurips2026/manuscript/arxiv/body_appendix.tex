...
\paragraph{C.1.1 Fold-0 primary result (Run
033)}\label{c.1.1-fold-0-primary-result-run-033}
...
result. These use clip-weighted mean $\times$ fixed-110-min denorm (the main-text within-center result
...
window was a 1.93-min penalty, see the main-text cross-center result). The
methodological point is that protocol choice changes both the magnitude
...
\section{Appendix F --- Shuffled-token semantic control (implementation and result)}\label{appendix-f-shuffled-token-semantic-control-implementation-and-result}

The result for this control appears inline in Appendix Z:
shuffled 13.14 $\pm$ 0.15 $\approx$ no-token 13.03 $\pm$ 0.18; $\neq$ decoupled-oracle 12.18
$\pm$ 0.14. Per-seed values are 13.25, 12.93, 13.24. This appendix records
the implementation details needed to reproduce the experiment.
...
variance. The control therefore supports the intended interpretation:
the cluster assignment has to be semantically correct to help.
...
main paper to fit the 9-page limit. The primary empirical claims and
strict-protocol result tables remain in the main paper; the supplement
provides controls, ablations, methodological details, and extended
discussion.
...
\tightlist
\item
  \textbf{The variability-scaling hypothesis as a cross-dataset
  prediction} --- that the marginal value of workflow
...
teacher-forced prefix, and pixel-only causal. In this pipeline,
``removing the oracle'' refers to two separate dependencies:

\begin{itemize}
\tightlist
\item
  \textbf{Inference-time oracle removed.} In the pixel-only causal
  evaluation, the cluster posterior is derived from the model's own
  phase-head predictions on the observed prefix. No ground-truth phase
  labels or full-video cluster labels are used at inference. The
  resulting deployable predictor beats no-token by \textbf{$-$0.18 min
  within-center} and \textbf{$-$0.22 min cross-center} under the matched
  per-video metric.
\item
  \textbf{Training-time oracle remains.} Training still consumes the
  full-video cluster ID at every clip, so the representations are learned
  under privileged information. A train-test alignment that uses
  prefix-derived clusters at training as well is
...
procedure. This narrowness makes Cholec80 a useful
\emph{low-variability} contrast for the variability-scaling claim, and
it explains why the main-text Cholec80 results show a null effect of
workflow conditioning on this dataset.
...
\textbf{directional rather than directly comparable}, and we treat them as such
...
in RSDNet \citep{twinanda2019rsdnet} and in TransLocal \citep{loukas2024translocal}. Per-video MAE in minutes on a held-out split is the reported
...
this paper's main results, see the main-paper limitations).
...
empirical argument: \textbf{primary results} (the central
variability-scaling claim), \textbf{controls} (rule out alternative explanations), and
\textbf{comparators} (legacy and absolute references).

\paragraph{Primary experiments --- tests of the variability-scaling
claim}\label{primary-experiments-tests-of-the-variability-scaling-claim}
...
\textbf{$-$0.85 min decoupled vs.~no-token (12.18 vs.~13.03)} & main-text within-center result \\
...
\textbf{$-$0.47 min decoupled vs.~no-token (17.33 vs.~17.80)} & main-text cross-center result \\
...
& Appendix Z --- shuffled-token control \\
...
\paragraph{Primary results at a
glance}\label{primary-results-at-a-glance}
...
below RSDNet $\approx$ 8.0 (2018) and TransLocal 7.10 (2024) & caveated
absolute reference \\
\end{longtable}
...
MultiBypass140} with the decoupled-phase-head architecture; the
shuffled-token control shows that the gain comes from correct workflow
assignment rather than parameter capacity; the same conditioning is statistically null on
...
versus the default sequence length 8 and stride 5 ($\approx$10 s). Single-seed
(seed 42), preliminary:
...
row closes the phase/cluster circularity diagnosed in the main-paper limitations and is the best
...
A natural concern with the main-text within-center result is whether the
...
This control closes one of the strongest robustness gaps that an
...
a lucky seed being surfaced. We report this explicitly because an ensemble
could otherwise be mistaken for cherry-picking; that is not what is
happening here.
...
\item
  ...
  stack, but it weakens comparability relative to the strict-protocol
...
If we temporarily ignore deployability and split-comparability caveats
and ask which component reduces MAE the most on each dataset, the answer
...
contribution of this paper is empirical: the variability-scaling claim,
the strict-protocol contrast, the shuffled-token control, and the
deployable pixel-only causal evaluation. The
...
the cluster posterior is most informative. We treat this as a diagnostic
observation rather than a primary result.
...
The released scripts reproduce the strict-protocol within-center result
(Run 033), the strict-protocol cross-center result (Run 034), and the
shuffled-token control (Run 037). The package includes:
...
A follow-up author with one GH-class GPU can reproduce any
...
\textbf{Reproducibility package.} The code and reproducibility artifacts
are available in the public
\href{https://github.com/billchenxi/BariatricRSD}{BariatricRSD repository}.
The package contains pinned \texttt{requirements.txt} (notably
...
\href{https://github.com/billchenxi/BariatricRSD/blob/master/reproducibility/QUICKSTART.md}{\texttt{QUICKSTART.md}}
with the six-step recipe from a fresh machine to reproducing the main
numbers, gzipped label JSONs (\textasciitilde37 MB compressed), and
verification scripts:

\begin{itemize}
\tightlist
\item
  \href{https://github.com/billchenxi/BariatricRSD/blob/master/reproducibility/scripts/verify_cholec80_3.56.sh}{\texttt{scripts/verify\_cholec80\_3.56.sh}}
...
\item
  \href{https://github.com/billchenxi/BariatricRSD/blob/master/reproducibility/scripts/verify_mb140_strict.sh}{\texttt{scripts/verify\_mb140\_strict.sh}}
  --- reproduces the strict prefix-only within-center numbers. Each of
  the three conditions (no-token, oracle, decoupled) evaluates each of
  three seeds and reports mean $\pm$ std with a pass/fail check against
  the manuscript values.
\end{itemize}

Trained checkpoints are deposited at the public
\href{https://huggingface.co/billchenxi/surgical-workflow-models}{Hugging Face repository}
(12 PyTorch state-dicts, \textasciitilde1.4 GB each, \textasciitilde17 GB
total). The download script in the package SHA256-verifies each file
against \texttt{weights/manifest.json} before loading, so readers can be
...
A reasonable concern when reading the main Results section is that the
negative rows on Cholec80 and under the legacy centered-window
cross-center protocol might weaken the positive within-center
MultiBypass140 result. We interpret them as part of the same
variability-scaling test. If workflow conditioning matters most when the
dataset contains real workflow variation, then we should see a benefit
on MB140, little or negative benefit on Cholec80, and sensitivity to
protocols whose artifacts are large relative to the workflow signal.
That is the pattern observed with the same model family and training
recipe. The Cholec80 results are consistent with limited residual
workflow signal, since 71\% of videos share one cluster. The
centered-window cross-center failure points to a protocol artifact that
the strict prefix-only protocol avoids.

Reporting only the within-center strict-protocol MB140 improvement would
make a weaker single-benchmark argument. The broader evidence includes
strict-protocol gains on within-center MB140 ($-$0.85 min, decoupled vs
no-token) and cross-center MB140 ($-$0.47 min), a shuffled-token control
showing that the gain is tied to workflow meaning rather than token
capacity, and null/negative Cholec80 results where the workflow
distribution is nearly collapsed. The claim is therefore best read as a
dataset-sensitive result: workflow tokens help when the dataset contains
workflow variation and provide little value when it does not.
...
hold for stronger backbones equipped with a workflow-conditioning input.
...
yields $-$0.47 min over no-token (17.33 vs 17.80), while the legacy
centered-window protocol produced a +1.93 min teacher-forced-prefix
failure. The residual $\approx$ 5-min
...
we frame the work as a benchmark-sensitive evaluation study rather than
as a universal architecture improvement claim.

The paper does \emph{not} establish a canonical-split Cholec80
leaderboard comparison: the 3.56 min test MAE is measured on a
non-canonical split and depends mostly on inference-time post-processing
that is orthogonal to workflow conditioning. It also does not establish a
fully circularity-free pixel-only causal estimate, because the phase head
is conditioned on the workflow token. We partially address this issue
with the strict decoupled-head condition, the shuffled-token control, and
the follow-up pixel-only causal evaluator released with the paper.
